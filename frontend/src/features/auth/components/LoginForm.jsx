...
const PROJECT_NAME =
  'Evaluación de fuentes generadoras de contaminación sonora en la UPC';
...
function DbMeter({ value = 62.4, limit = 65 }) {
  ...
  const limPct = (limit / 120) * 100;
  const bars   = [38, 70, 52, 88, 61, 100, 74, 46, 92, 58, 80, 48];
  return (
    <div style={{ width: '100%' }}>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginBottom: 14 }}>
        <span style={{ fontFamily: FD, fontSize: 52, fontWeight: 700, color: 'rgba(255,255,255,0.92)', lineHeight: 1, letterSpacing: '-0.02em' }}>
          {value.toFixed(1)}
        </span>
        ...
        <span style={{ marginLeft: 'auto', fontFamily: FB, fontSize: 12, fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', color: C.naranja, display: 'flex', alignItems: 'center', gap: 6 }}>
          <span style={{ width: 7, height: 7, borderRadius: 99, background: C.naranja, boxShadow: `0 0 8px ${C.naranja}`, animation: 'acu-blink 1.6s ease-in-out infinite' }} />
          En vivo
        </span>
      </div>
      <div style={{ display: 'flex', alignItems: 'flex-end', gap: 4, height: 46, marginBottom: 16 }}>
        {bars.map((h, i) => (
          <div key={i} style={{
            ...
            background: `linear-gradient(${C.verde}, ${C.azul})`,
            transformOrigin: 'bottom',
            animation: `acu-eq 1.${3 + (i % 6)}s ${i * 0.08}s ease-in-out infinite alternate`,
          }} />
        ))}
      </div>
      ...
      <div style={{ position: 'relative', marginTop: 7, fontFamily: FB, fontSize: 11.5, color: 'rgba(255,255,255,0.55)', display: 'flex', justifyContent: 'space-between' }}>
        <span>0</span>
        <span style={{ position: 'absolute', left: `${limPct}%`, transform: 'translateX(-50%)', color: 'rgba(255,255,255,0.92)', fontWeight: 600 }}>
          límite {limit}
        </span>
        <span>120</span>
      </div>
...
export default function LoginForm() {
  ...
  const Panel = (
    <div style={{
      position: 'relative', overflow: 'hidden', background: '#0f5840',
      backgroundImage: 'radial-gradient(120% 120% at 18% 12%, #1a7a58 0%, #0f5840 48%, #0a4131 100%)',
      color: '#fff',
      flex: isMobile ? 'none' : '1 1 56%',
      height: isMobile ? 200 : 'auto',
      padding: isMobile ? '22px 26px' : '52px 56px',
      ...
      <div style={{ position: 'relative', zIndex: 1, display: 'flex', alignItems: 'center', gap: 13 }}>
        ...
        <div>
          ...
          <div style={{ fontFamily: FB, fontSize: 11.5, fontWeight: 600, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.62)' }}>
            Sistema BI · Monitoreo de ruido
          </div>
        </div>
      </div>
      {!isMobile && (
        <div style={{ position: 'relative', zIndex: 1, maxWidth: 460 }}>
          <h2 style={{ fontFamily: FD, fontWeight: 600, fontSize: 27, lineHeight: 1.2, letterSpacing: '-0.02em', margin: '0 0 28px' }}>
            {PROJECT_NAME}
          </h2>
          <div style={{ background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 18, padding: '22px 24px', backdropFilter: 'blur(4px)' }}>
            <div style={{ fontFamily: FB, fontSize: 12.5, fontWeight: 600, letterSpacing: '0.05em', color: 'rgba(255,255,255,0.6)', marginBottom: 14, textTransform: 'uppercase' }}>
              Estación Centro · UPC Valledupar
            </div>
      ...
      <div style={{ position: 'relative', zIndex: 1, display: isMobile ? 'none' : 'flex', alignItems: 'center', gap: 9, fontFamily: FB, fontSize: 12.5, color: 'rgba(255,255,255,0.7)' }}>
        <span style={{ display: 'inline-flex', padding: '5px 11px', borderRadius: 99, background: 'rgba(240,160,42,0.16)', border: '1px solid rgba(240,160,42,0.35)', color: C.naranja, fontWeight: 600 }}>
  ...
  const Form = (
    <div style={{
      flex: isMobile ? 'none' : '1 1 44%',
      background: '#F7F5EF',
      padding: isMobile ? '32px 26px 40px' : '56px clamp(40px, 6vw, 76px)',
      display: 'flex', flexDirection: 'column', justifyContent: 'center',
    }}>
      <div style={{ width: '100%', maxWidth: 380, margin: '0 auto' }}>
        <h1 style={{ fontFamily: FD, fontWeight: 700, fontSize: isMobile ? 26 : 30, color: '#13231c', letterSpacing: '-0.02em', margin: '0 0 6px' }}>
          Iniciar sesión
        </h1>
        <p style={{ fontFamily: FB, fontSize: 15, color: '#5a6962', margin: '0 0 30px' }}>
          Accede al panel de monitoreo acústico.
        </p>
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
          ...
          />
          {error && (
            <div style={{ fontFamily: FB, fontSize: 13.5, color: '#c0492a', fontWeight: 600, marginTop: -4 }}>
              {error}
            </div>
          )}
